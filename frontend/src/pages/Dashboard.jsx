...
function Dashboard() {
   ...
   const handleUpdate = async () => {
    ...
    try {
        const token = localStorage.getItem("token")
        const id = editingTodo._id || editingTodo.id  // ← dono check karo
    ...
    // 4 DELETE — URL fix: /todo/${id} → /todo/delete/${id}
    const handleDelete = async (id) => {
        try {
            const token = localStorage.getItem("token")
            await axios.delete(
                `http://localhost:3000/todo/delete/${id}`,  // ← fix
                {
                    withCredentials: true,
                    headers: { Authorization: `Bearer ${token}` }  // ← space fix
                }
            )
...
const handleToggle = async (todo) => {
    ...
    return (
        <div className="min-h-screen bg-black text-white">
            ...
            {/* NAVBAR*/}
            <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 flex items-center justify-between px-6 h-14 ${scrolled ? "bg-black/80 backdrop-blur border-b border-gray-800" : "bg-transparent"
                }`}>
            ...
            <div className="max-w-2xl mx-auto px-6 pt-24 pb-24">
                <h1 className="text-lg font-serif mb-6">Welcome, <em className="text-fuchsia-400 italic">{username}</em></h1>
                ...
                {showForm && (
                    ...
                    >
                        <div className="w-full max-w-md bg-gray-900 border border-gray-800 rounded-2xl p-7 shadow-2xl">
                            ...
                            <div className="mb-6">
                                <label className="block text-xs text-gray-500 mb-2">Description <span className="text-gray-700">(optional)</span></label>
                                <textarea
                                    value={description}
                ...
                {todos.map(todo => (
                    <div key={todo._id || todo.id} className="flex items-start gap-4 bg-gray-900 border border-gray-800 rounded-xl px-5 py-4 mb-3">
        ...
        <button
            ...
            className={`mt-0.5 w-5 h-5 rounded-full border shrink-0 flex items-center justify-center transition-all ${
                todo.completed ? "bg-indigo-600 border-indigo-600" : "border-gray-600 hover:border-indigo-400"
            }`}
        >
            {todo.completed && (
                <svg width="9" height="9" viewBox="0 0 10 10" fill="none" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M2 5l2.5 2.5L8 3"/>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {todo.description && <p className="text-gray-500 text-xs mt-1">{todo.description}</p>}
                        </div>
                        ...
                        <div className="flex gap-1 shrink-0">
                            <button
                                ...
                                className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-600 hover:text-indigo-500 hover:bg-gray-800 transition-colors"
                            >
